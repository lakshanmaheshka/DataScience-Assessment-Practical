...
MongoClient.connect(
    ...
    (error, client) => {
        ...
        db.collection('CarFuelAndEmissions').find({$and:[{fuel_type:'Diesel'}, { $expr: { $gt: [ { $toDouble: "$co2_emissions" }, 100.0 ] } },{ $expr: { $lte: [ { $toDouble: "$co2_emissions" }, 150.0 ] } } ]}).toArray(
            (error, CarFuelAndEmissions) => {
                // console.log(CO2EmissionFromNaturalGas)
                CarFuelAndEmissions.slice(0, 5).forEach(function(CarFuelAndEmissions){
